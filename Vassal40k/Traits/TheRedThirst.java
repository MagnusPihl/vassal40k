...
public class TheRedThirst extends Decorator implements TranslatablePiece
{
    ...
    protected KeyStroke keyStroke = KeyStroke.getKeyStroke(KeyEvent.VK_R, KeyEvent.CTRL_DOWN_MASK | KeyEvent.ALT_DOWN_MASK);
    ...
    protected KeyCommand[] myGetKeyCommands()
    {
        if (keyCommand == null)
        {
            ...
            keyCommand[0] = new KeyCommand("The Red Thirst", keyStroke, this);
        }
        return keyCommand;
    }
    ...
    {
        if (myGetKeyCommands()[0] != null && myGetKeyCommands()[0].matches(ks))
        {
            ...
            if (!hasRolled)
            {
                ...
                if (dice == 6)
                {
                    ...
                    keyCommand[0] = new KeyCommand("Has the Red Thirst (click to reset)", keyStroke, this);
                }
                else
                {
                    hasRedThirst = false;
                    WriteLine("* (" + dice + "): " + getName() + " does NOT have The Red Thirst!");
                    keyCommand[0] = new KeyCommand("Does not have the Red Thirst (click to reset)", keyStroke, this);
                }
                hasRolled = true;
            }
            else
